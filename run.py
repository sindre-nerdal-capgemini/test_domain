...
parser.add_argument('--domain', '-d', help="The domain to run requests agains. I.E https.example.com", type= str, default='https://ndla.no')
...
if __name__ == '__main__':
    ...
    for request_in_parallel in requests_in_parallel:
        if generate_pdf_report:
            test = ArchiveSpoltest(domain, request_in_parallel, test_request_count_after_cached, should_follow_redirects, meta_file, timeout_between_each_chunk, OUTPUT_DIR)
            test.run()
        if loggly_token:
            test =  LogglySpoltest(domain, request_in_parallel, test_request_count_after_cached, should_follow_redirects, meta_file, timeout_between_each_chunk, OUTPUT_DIR, LOGGLY_URL)
            test.run()
